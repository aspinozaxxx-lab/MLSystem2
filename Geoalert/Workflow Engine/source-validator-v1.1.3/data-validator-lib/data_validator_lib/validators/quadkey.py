...
from ..errors import quadkey as quadkey_error, basemap as basemap_error


class QuadkeyValidator(BasemapValidator):
    """
    Validator for Quadkey basemap. Currently implements check for the url in the input request

    Checks if the request contains valid url, that is string starting with http:// or https://
    and contains {q} placeholder

    Checks the request params: zoom to be in the specified range

    correct input: {'source_type':'quadkey', 'url':'https://basemap.url.com/{q}.png'}

    """
    def _request_is_ok(self, request: dict) -> Tuple[Status, Optional[ErrorMessage]]:
        """
        Checks if the request contains valid url, that is string starting with http:// or https://
        and contains valid quadkey placeholder {q}
        Args:
        ...
        if source_type != 'quadkey':
            raise RuntimeError(f'Quadkey validator called with wrong {source_type = }')
        ...
        if not ['q'] == format_keys:
            return Status.ERROR, quadkey_error.QuadkeyLinkFormatError()
        else:

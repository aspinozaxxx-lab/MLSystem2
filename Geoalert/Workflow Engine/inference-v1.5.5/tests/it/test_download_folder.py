...
def test_download_folder(create_test_files):
    ...
    upload_artifact = InferenceArtifact(path=s3_path, name=filename1.name)
    ...
    upload_artifact = InferenceArtifact(path=s3_path, name=filename2.name)
    upload_create_bucket(storage=storage, artifact=upload_artifact, workdir=workdir, bucket_name=folder_bucket_name)
    ...
    upload_artifact = InferenceArtifact(path=s3_path, name="tmp.jpg")
    upload_create_bucket(storage=storage, artifact=upload_artifact, workdir=workdir, bucket_name=folder_bucket_name)
    ...
    download_artifact = InferenceArtifact(path=f's3://{folder_bucket_name}/', name="out5.tif")
    ...
    # download
    storage.get_folder_raster_artifact(artifact=download_artifact, workdir=workdir, aoi=aoi)

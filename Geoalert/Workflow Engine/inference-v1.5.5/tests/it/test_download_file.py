...
from inference.errors import S3DownloadError

minio_host = 'localhost'
minio_port = '9000'
minio_secret_key = 
minio_access_key = 
BUCKET_NAME = 'healthcheck'
...
def create_test_files():
    config = InferenceConfig(MINIO_HOST=minio_host, MINIO_URL=minio_url, MINIO_PORT=minio_port,
...
def upload_create_bucket(storage, artifact, workdir, bucket_name):
    ...
    try:
        ...
        pass
    logger.debug(f"Uploading artifact {artifact.name} to {artifact.path}, {type(artifact.path)}")
...
def test_download_whole_nocut(create_test_files):
    ...
    upload_artifact = InferenceArtifact(path=s3_path, name=filename)
    storage = InferenceStorage(minio_url=minio_url,
                               minio_secret_key=,
                               minio_access_key=
    upload_create_bucket(storage=storage, artifact=upload_artifact, workdir=workdir, bucket_name=BUCKET_NAME)
    download_artifact = InferenceArtifact(path=s3_path, name='out.tif')
...
def test_download_partial(create_test_files):
    ...
    download_artifact = InferenceArtifact(path=s3_path, name="out5.tif")
...
def test_download_error(create_test_files):
    logger.debug("TESTING DOWNLOAD WITH WRONG PATH")

    workdir, filename, aoi = create_test_files
    s3_path = 's3://healthcheck/healthcheck.tif'
    wrong_file_path = 's3://healthcheck/not-exists.tif'
    folder_path = 's3://healthcheck/'
    wrong_aoi = shapely.affinity.translate(aoi, 1.0, 1.0)
    upload_artifact = InferenceArtifact(path=s3_path, name=filename)
    storage = InferenceStorage(minio_url=minio_url,
                               minio_secret_key=,
                               minio_access_key=
    ...
    download_artifact = InferenceArtifact(path=wrong_file_path, name='out.tif')
    with pytest.raises(S3DownloadError, match='The specified key does not exist') as e:
        storage.get_artifact(artifact=download_artifact, workdir="/")

    folder_artifact = InferenceArtifact(path=folder_path, name='out.tif')
    with pytest.raises(S3DownloadError, match='No files intersect with AOI') as e:
        storage.get_artifact(artifact=folder_artifact, workdir="/", aoi=wrong_aoi)

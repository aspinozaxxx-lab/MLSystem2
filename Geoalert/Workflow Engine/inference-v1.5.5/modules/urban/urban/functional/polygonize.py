...
from rasterio.transform import IDENTITY, xy, Affine
from shapely.geometry import shape, Polygon, MultiPolygon, GeometryCollection
from gpdadapter import FeatureCollection
from typing import Optional, Sequence, List


def polygonize(sample,
               epsilon=0.1,
               properties: Optional[dict] = None):
    """ Transform the raster mask to vector polygons.
    The pixels in the raster mask are treated as belonging to the object if their value is non-zero, and zero values are
    background.
    ...
    This method is used as it does process the hierarchy of inlaid contours correctly.
    ...
    Args:
        sample: BandSample to be vectorized (must be 8bit unsigned int)
    ...
    """
    polys = _extract_polygons(_vectorize(sample.numpy(), epsilon=epsilon, transform=sample.transform))
    features = {k: [v]*len(polys) for k, v in properties.items()} if properties else dict()
    features['geometry'] = polys
    return FeatureCollection(features, crs=sample.crs)


def _extract_polygons(geometries: Sequence[dict]) -> List[Polygon]:
    """
    ...
    for geom in geometries:
        if not geom['type'] in ['Polygon', 'MultiPolygon']:
            ...
            if isinstance(new_shape, Polygon):
                ...
                shapes += new_shape.geoms
            elif isinstance(new_shape, GeometryCollection):
                for sh in new_shape.geoms:
...
def _vectorize(binary_image: np.ndarray,
               epsilon: float = 0.,
               min_area: float = 1.,
               transform: Affine = IDENTITY,
               upscale: float = 1) -> List[dict]:
    """
    Vectorize binary image, returns a 4-level list of floats [[[[X,Y]]]]

    Args:
        binary_image (numpy.ndarray): image in grayscale HxW or HxWx1
        epsilon (float): approximates a contour shape to another shape with the smaller
    ...
    # approximate contours with smaller number of points
    if epsilon > 0.:

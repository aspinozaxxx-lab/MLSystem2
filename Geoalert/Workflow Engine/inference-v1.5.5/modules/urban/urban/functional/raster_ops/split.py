...
def _check_channels_num(src, channels, dst_channels, allow_singleband):
    ...
    if src_channels > dst_channels:
        if src_channels == dst_channels + 1 and src.colorinterp[-1] == ColorInterp.alpha:
            ...
            logger.warning(
                (
                    "Input raster expected to have {} ({}) channels, but got {} channels. "
                    "Only first {} channels would be used."
                ).format(dst_channels, channels, src_channels, dst_channels)
            )
    elif singleband:
        logger.warning(
            (
                "Input raster has 1 channel, but we need {} channels. "
                "Due allow_singleband is set True, first channel would be copied to produce {} channels."
            ).format(dst_channels, dst_channels)
        )
    elif src_channels < dst_channels:
        raise ValueError(
            "Input raster expected to have {} ({}) channels, but got {} channels".format(
                dst_channels, channels, src_channels,
            )
        )
    return singleband
...
def _get_nodata(src: rasterio.DatasetReader, band=0):
    ...
    if MaskFlags.nodata in src.mask_flag_enums[band]:
        ...
        if len(alpha) == 1:
            ...
            logger.warning("Alpha channel assumed by MaskFlags but not present in ColorInterp. Returning without nodata")
        else:  # len(alpha) > 1
            logger.warning(f"There are {len(alpha)} Alpha-channels in the dataset which is unexpected; we will not use them")
        return None, False
        ...
        # TODO: change nodata mask to internal dataset bitmasks all library-wise
        return 0, True
    else:
        # All_valid
        return None, False
...
def split(src_fp,
    ...
    Args:
        ...
        exist_ok: set True to not rise error, if output path already exists
    ...
    with rasterio.open(src_fp) as src:
        ...
        # a doubtful thing: we use the read_mask flag per-dataset, whereas masks may be per-channel
        ...
        for out_band in tqdm(range(dst_channels)):
            ...
            dst_paths.append(dst_band_path)

    return BandCollection(dst_paths)

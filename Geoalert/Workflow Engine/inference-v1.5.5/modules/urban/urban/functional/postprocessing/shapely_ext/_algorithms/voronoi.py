...
def angle_between_pts(a: np.ndarray, b: np.ndarray, ref_vec=(1.0, 0.0)):
    """
...
def polygon_around_center(points: np.ndarray, center: Optional[np.ndarray] = None, fix_nan_angles=True):
    """
...
def voronoi_regions_from_coords(coords,
                                geo_shape: shapely.Polygon,
...
def polygon_lines_from_voronoi(vor, geo_shape, return_only_poly_lines=True, farpoints_max_extend_factor=10):
    ...
    center = np.array(MultiPoint(vor.points).convex_hull.centroid.coords)[0]

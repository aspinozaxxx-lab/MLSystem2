...
from mlsystem2.training_ui_api._datasets import list_classes


def test_list_classes_uses_git_history_for_dataset_update_dates(tmp_path: Path) -> None:
    mlmarkup_root = tmp_path / "MLMarkup"
    class_a = mlmarkup_root / "class_a"
    class_b = mlmarkup_root / "class_b"
    class_a.mkdir(parents=True)
    class_b.mkdir(parents=True)
    (class_a / "class_a.txt").write_text("scene-a\n", encoding="utf-8")
    (class_a / "class_a.geojson").write_text("{}", encoding="utf-8")
    (class_b / "class_b.txt").write_text("scene-b\n", encoding="utf-8")
    (class_b / "class_b.geojson").write_text("{}", encoding="utf-8")
    ...
    (class_b / "class_b.geojson").write_text('{"updated":true}', encoding="utf-8")
    _git(["add", "class_b/class_b.geojson"], mlmarkup_root)
    _git(

...
Редактор читает и коммитит отдельный SSH-клон, никогда не пишет в live-каталог. Frontend хранит черновики нескольких
снимков в памяти вкладки; `PUT /api/v1/dataset-editor/datasets/{dataset_key}/scenes` проверяет их все и публикует одним атомарным commit.
Совместимый одиночный save меняет один GeoJSON; добавление прямых TIFF папки создаёт один commit; delete удаляет
одну сцену. Все Git-операции сериализованы,
...
`dataset_mask` TIFF, возвращает его как `valid_data_footprint`, обрезает им разметку для редактирования и проверяет
CRS снимка, Polygon/MultiPolygon, валидность и полное попадание в этот контур. OpenLayers отключает интерполяцию
...
в скачиваемый ZIP и прямой расчёт F1; полное удаление разметки удаляет запись и весь каталог файлов. Итоговые
...
редактор ищет среди всех готовых результатов текущей основной сети тот, чей `scenes_file` содержит выбранный TIFF;

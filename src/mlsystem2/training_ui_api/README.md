...
Псевдоразметка эффективной сети с полным фактическим покрытием снимков используется только для создания,
`evaluate-preview`, `optimize-preview` и поснимочного кэша оптимизатора. Точное совпадение датасета предпочтительно,
но не обязательно; новый pseudo result не перезаписывает контрольные метрики.

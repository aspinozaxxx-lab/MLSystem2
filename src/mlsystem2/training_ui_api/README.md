...
внутренние компоненты проходят полную постобработку. Метрика устаревает при смене выборки, ревизии состава,
профиля, эффективного шаблона или версии алгоритма оценки; успешная новая сеть ставится на оценку автоматически,
а массовый ручной запуск выполняет `POST /api/v1/results/classes/{class_key}/test-f1`. В MLflow эти метрики не
записываются.

...
- `MLSYSTEM2_GRAFANA_URL`, `MLSYSTEM2_MLFLOW_UI_URL` — ссылки на UI сервисов.
- `MLSYSTEM2_IMAGES_UI_URL` — ссылка на авторизованный браузер подготовленных снимков только для чтения;
  значение по умолчанию — `/prepared-images/`.
...
Каталог группирует тестовые разметки по классу. Выключенные тайлы остаются доступными для возврата, но не входят
...
Назначение звездой уже используемой последней сети меняет только явную ссылку класса и не создаёт заданий.
При фактической смене эффективной сети пересчитываются сохранённые контрольные метрики, но независимая матрица
F1 остальных результатов обучения не затрагивается.
...
При создании фиксируются сеть, обученная на выбранном исходном датасете, и её полная псевдоразметка с тем же
`dataset_key` и фактическим покрытием снимков. Именно эта пара используется для `evaluate-preview`,
`optimize-preview` и поснимочного кэша оптимизатора; результат другого датасета класса не подставляется. Новый
pseudo result не перезаписывает контрольные метрики.
...
достижимость итоговых ограничений и по зафиксированной точной псевдоразметке выбранной пары датасет-сеть включает состав внутри диапазона
...
`GET /api/v1/test-sample-batches/options` группирует варианты по классам и для каждого датасета показывает его
выбранную успешную сеть и состояние точной псевдоразметки. Отсутствующий результат идемпотентно ставится через
`POST /api/v1/test-sample-batches/options/{dataset_key}/pseudo-markup`.
...
автоматически ставят оценки трёх последних успешных сетей каждого датасета, старт сервиса восстанавливает тот же
ограниченный набор, а `POST /api/v1/results/datasets/{dataset_key}/test-f1` ставит все успешные сети выбранного
датасета. Для управляемой сети изменение выборки одного исходного класса пересчитывает только его канал и
объединяет результат с сохранёнными метриками остальных классов; без полной сохранённой структуры выполняется
полный расчёт. В MLflow эти метрики не
записываются.
Размер самого тестового TIFF не используется как размер входа модели. Нативный runner сначала читает
`sample_size`, `inference_context` и полезный центр из metadata checkpoint, для старого checkpoint использует
конфигурацию исходного training-job и этими окнами покрывает всю площадь TIFF. В задании F1 `tile_size` и поля
`inference_*` означают фактическую геометрию окна сети.
...
фильтрами и теряют служебный тег после завершения цепочки. Для `Озера\main` и `smp_segformer_b2` seed-шаблон
включает обратный режим `keep_compact`, который экспортирует `FilterNonCompactObjects(0.25, 3.5)` без
сглаживания. В базовом шаблоне compact-фильтр и Smooth выключены. Старые настройки без поля
`postprocess.filter_compact_objects.mode` продолжают работать в режиме `remove_compact`.
...
Срочная поснимочная псевдоразметка редактора и все расчёты F1 используют ту же inference-очередь с
признаком `priority=urgent`. Срочное queued-задание обгоняет обучение независимо от источника задания.
...
`GET /api/v1/results/classes` выделяет основной датасет и для каждой карточки выбирает сеть этого датасета:
явно отмеченную основную, если она принадлежит ему, либо последнюю успешную. Её существующий F1 берётся по общей
основной тестовой разметке класса; основная сеть другого датасета значение карточки не подменяет.
...
шаблон может ужесточить или ослабить только нужные параметры. Включенный `filter_compact_objects` работает
независимо от площади по isoperimetric quotient и отношению сторон minimum rotated rectangle. Режим
`remove_compact` удаляет компактные полигоны и используется для рек, а `keep_compact` оставляет их и используется
для озёр. Полигоны в пределах одного пикселя от границы растра сохраняются в обоих режимах; невалидные геометрии
не приводят к ошибке фильтрации. Затем речной профиль выполняет Smooth перед Simplify.

...
`run_train_pipeline` получает settings, открывает MLflow run, пишет YAML-конфиг запуска и вызывает `dataset_preparing`. После отчета подготовки датасета создает train/val DataLoader и оборачивает их внутренним счетчиком tile batches, augmented/positive tiles и диагностик VRT source rects. Если `settings.train.initial_checkpoint_uri` задан, вызывается `models.load_checkpoint` с `LoadCheckpointRequest`; иначе вызывается `models.create_model`.

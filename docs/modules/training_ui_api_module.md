...
`Z`, поэтому одинаково принимает `Ctrl+Z` и `Ctrl+Я`. Кнопка карты раскрывает рабочее поле с инструментами в
браузерный полноэкранный режим, `Esc` возвращает страницу, а OpenLayers после перехода пересчитывает размер карты.

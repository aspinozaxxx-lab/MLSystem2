...
HF SegFormer B0 допускает фиксированный `next-gen2` из segFormer_train_hlam_main_v2.ipynb.
Профиль задаётся ConfigSchema.pipeline_defaults; подробное описание — pipeline_descriptions.next_gen2.
Доступны только максимум эпох (20), patience по validation loss (10) и лимит времени (null).
Форма показывает модель/датасет, условия остановки и описание внизу с визуализацией тайла.
Серверная обработка шаблонов и worker закрепляют остальные значения: 512/256, аугментации,
batch 16, LR 1e-4, CrossEntropy + Tversky, seed 42, workers 0. Settings проверяет профиль.

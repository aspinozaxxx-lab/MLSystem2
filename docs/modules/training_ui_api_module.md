...
При чтении каталога сервис транзакционно добавляет неизвестные папки MLMarkup, сохраняет существующие `dataset_key` и ручные настройки. Класс содержит датасеты и единый тип снимков: `kanopus` читает четыре канала `uint8` из `prepared_images/kanopus`, `ortho` — RGB `uint8` из `prepared_images/orto`; строка-папка в TXT разворачивается внутри этого корня. Worker создаёт и запускает training/inference jobs через общую очередь и публичные фасады модулей, а метрики обучения пишет только `train_pipeline` в MLflow. Число каналов сохраняется в job, передаётся модели, проверяется при подготовке TIFF и сверяется с checkpoint при инференсе без обрезки или дополнения. Тестовые разметки формируются отдельно от очереди; ZIP для RGB содержит только RGB-превью, для RGB+NIR — RGB/NRG/NGB, каждый JPEG не больше 300 KiB.

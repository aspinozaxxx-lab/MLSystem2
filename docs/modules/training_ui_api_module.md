...
по txt того же датасета. Manual jobs имеют приоритет выше automation jobs. Auto jobs нельзя удалить или двигать

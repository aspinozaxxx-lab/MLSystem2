...
- `GET /api/v1/test-samples/primary/download` - ZIP основных разметок с отдельной папкой датасета.
...
При чтении каталога сервис транзакционно добавляет неизвестные папки MLMarkup, сохраняет существующие `dataset_key` и ручные настройки. Класс содержит датасеты и единый тип снимков: `kanopus` читает четыре канала `uint8` из `prepared_images/kanopus`, `ortho` — RGB `uint8` из `prepared_images/orto`; строка-папка в TXT разворачивается внутри этого корня. Worker создаёт и запускает training/inference jobs через общую очередь и публичные фасады модулей, а метрики обучения пишет только `train_pipeline` в MLflow. Число каналов сохраняется в job, передаётся модели и строго проверяется при подготовке TIFF; одноразовая псевдоразметка и тестовый F1 дополнительно принимают RGBA для трёхканального checkpoint, читая только первые три канала, а остальные несовпадения отклоняют. Inference-шаблон выбирается по датасету обученной модели, а не по источнику снимков. Тестовые разметки формируются отдельно от очереди; ZIP для RGB содержит только RGB-превью, для RGB+NIR — RGB/NRG/NGB, каждый JPEG не больше 300 KiB.

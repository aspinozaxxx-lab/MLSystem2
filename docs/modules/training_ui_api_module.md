...
- Запуск строится как «архитектура → конвейер». Все SegFormer каталога поддерживают `legacy` и
  `next_gen2`; остальные сети — `legacy`. HF B0 называется «SegFormer B0 HF» и по умолчанию использует
  `next_gen2`. Профили и batch для архитектуры передаются через `ConfigSchema.pipeline_defaults`.

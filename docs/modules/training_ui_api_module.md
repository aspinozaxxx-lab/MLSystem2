...
HF SegFormer B0 допускает выбор `next-gen2`. UI применяет профиль из
`ConfigSchema.pipeline_defaults`, скрывает несовместимые параметры и сохраняет выбор в обычном шаблоне/job.
Профиль сохраняет обработку изображений ноутбука, но нарезает полные тайлы без нахлёста: шаг автоматически
равен размеру тайла, отдельное поле скрыто. При новом запуске и сохранении шаблона старое значение шага
заменяется размером тайла. Доля validation доступна в редакторе (по умолчанию 20%) и рассчитывается
среди оставшихся после исключений тайлов с учётом целого размера выборки,
а из train исключаются все окна с общей площадью с validation любого TIFF. Best выбирается по F1.
Параметры новых запусков: до 300 эпох, batch 32,
ранняя остановка после 50 эпох без улучшения валидационной F1 и лимит времени 10800 секунд.
Явные настройки ранее сохранённых шаблонов остаются в силе.
API отклоняет несовместимые архитектуру, тип снимков/задачи и настройки; worker сохраняет CrossEntropy,
полную нарезку, seed 42 и заданное число workers. Исторические результаты сохраняют выбор best по loss.
...
`> 0.9`, batch 32. Диагностический порог validation `0.5` не меняется. Автоматическая морфология и упрощение по числу

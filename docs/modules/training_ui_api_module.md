...
- `POST /api/v1/test-samples/download` - несжатый ZIP явно выбранных сохранённых разметок; до восьми разметок готовятся параллельно, каждая в собственной папке архива.
...
При чтении каталога сервис транзакционно добавляет неизвестные папки MLMarkup и сохраняет `dataset_key`. Класс задаёт четыре канала `kanopus` либо RGB `ortho`; worker запускает training/inference через общую очередь, а метрики пишет только `train_pipeline`. Одноразовая псевдоразметка и тестовый F1 принимают RGBA для трёхканального checkpoint, читая RGB; inference-шаблон выбирается по датасету модели. Тестовые разметки формируются вне очереди: после смены CRS полигоны восстанавливаются через `make_valid`, допустимый пул сохраняется при тайм-ауте вторичной оптимизации, а невозможный минимум получает точный достижимый максимум. Одиночный ZIP содержит полный набор либо только TIFF/GeoJSON. Выбранные сохранённые разметки готовятся максимум восемью потоками в раздельных каталогах и последовательно собираются в `ZIP_STORED`; JPEG ограничен 300 KiB.

...
HF SegFormer B0 допускает `next-gen2` на основе segFormer_train_hlam_main_v2.ipynb для Канопус и RGB-ортофотопланов.
Профиль задаётся ConfigSchema.pipeline_defaults; подробное описание — pipeline_descriptions.next_gen2.
Доступны размер тайла 512/768/1024/1536 (default 512), максимум эпох (20), patience по validation loss (10) и лимит времени (null).
Форма показывает модель/датасет и размер, условия остановки и описание внизу с визуализацией тайла.
Обработка шаблонов и worker вычисляют шаг как половину размера и batch 16/8/4/2 соответственно.
...
`> 0.9`, batch 32/16/8/4 по размеру окна. Диагностический порог validation `0.5` не меняется. Автоматическая морфология и упрощение по числу

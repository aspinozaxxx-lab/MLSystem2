...
Binary mask rasterize выполняется в том же окне только по positive `annotation_file`, возвращает shape `1,H,W`, dtype `float32`, значения `0/1`. Multiclass mask rasterize загружает spatial index для каждого positive-класса, создает `int64 [H,W]`, а background оставляет `0`. Классы применяются по `(priority, class_id)`: меньший priority записывается раньше, больший priority перекрывает его в пересечениях; при равном priority более поздний `class_id` перекрывает более ранний. Hard-negative GeoJSON никогда не rasterize в target mask и всегда остается background `0`. Nodata pixels зануляются в background. Это делает пересечения разметки детерминированными без падения DataLoader.

Аугментация сохраняет raw Geoalert tensor ABI подготовленных снимков: после photometric значения image зажимаются в диапазон `0..255`. Geometric flips/rotations применяются к image и mask, photometric применяется только к image. Модуль не зануляет прямоугольные patch-и в image или mask. Категория tile определяется до аугментации и после нее не меняется. При `mode=train` и `augmentation_level > 0` аугментация применяется к positive и hard_negative tiles; обычный background не аугментируется. Для hard_negative tile в `apply_augmentations` передается нулевая mask, поэтому после аугментации target mask остается полностью нулевой. Sample meta содержит `augmented`, `category`, `positive`, `hard_negative`, `background`; collate собирает batch `(images, masks, batch_meta)` с category counters и per-tile flags.

Dataset всегда строит cheap-index для `train` и `val`: по bounds окна проверяет пересечение с GeoJSON geometry без чтения raster data и без rasterize. Для каждого tile задается ровно одна категория: `positive`, если rasterized target mask имеет positive pixels или positive hint пересекает окно; `hard_negative`, если positive отсутствует, но окно пересекает hard-negative геометрию; иначе `background`. При пересечении positive и hard-negative разметки категория всегда `positive`. В multiclass режиме positive hint означает пересечение с геометрией любого класса, а hard-negative hint строится по объединению hard-negative геометрий всех классов. Если задан `tile_split`, positive и non-positive hints делятся на train/val по `val_fraction` и `seed`; пересечения между subsets запрещены, редкие positive windows не дублируются, а предупреждения доступны в tile report. Поля `estimated_positive_tiles`, `estimated_hard_negative_tiles` и `estimated_background_tiles` являются geometry-intersection hint, а не точным rasterized mask count. Для `train` DataLoader использует `WeightedRandomSampler` с replacement и тремя бюджетами: `positive_factor`, `hard_negative_factor`, `background_factor`. Внутри каждой категории вес одного tile равен бюджету категории, деленному на число tiles категории; при `class_balance=true` только positive-бюджет распределяется между positive-классами. Если фактор категории больше нуля, а tiles этой категории нет, создание sampler завершается `TilePreparationError`; нулевой фактор отключает категорию. Для `val` train factors не применяются: выбирается фиксированный balanced subset по positive/non-positive hints, порядок детерминирован от `seed`, replacement не используется. Shuffle и augmentation для val выключены.

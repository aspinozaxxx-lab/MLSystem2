...
Отдельная ветка `next_gen2` строит полные окна без контекста и фильтрации nodata. `window_random`
выбирает 20% тайлов по SHA-256 от seed, scene_id и координат, независимо от разметки и порядка сцен.

...
Модуль не ищет соседние снимки, не использует footprints и не строит VRT. Он получает готовые VRT XML, открывает их in-memory через rasterio `MemoryFile` и возвращает источники батчей. Текущая нарезка батчей остается заглушкой: источники пустые, а реальное чтение окон из VRT будет следующим шагом.

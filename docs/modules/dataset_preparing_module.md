...
После сопоставления снимков модуль строит один общий VRT по всем найденным снимкам. Пул не делится по сценам: train/val split выполняется только в `tile_preparation` по тайлам, а scene report помечает найденные снимки как `pool`.

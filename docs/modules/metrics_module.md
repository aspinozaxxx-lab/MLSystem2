...
- `compute_object_f1(request: ObjectF1Request) -> ObjectF1Result` - считает объектовую F1 массива ID.
...
несопоставленные эталоны — FN. Таблица площадей и пересечений строится одним проходом полосами, без повторного сканирования массива для каждого объекта. Вызывающий модуль агрегирует TP/FP/FN. В object_f1 массив содержит целый снимок после сборки окон; в прежних режимах — отдельный тайл.

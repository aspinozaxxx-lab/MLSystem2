...
Multiclass режим требует `task=multiclass` и `loss=cross_entropy`. Logits имеют форму `[B,num_classes,H,W]`, mask - `[B,H,W] long`. Loss считается через `torch.nn.functional.cross_entropy`. Validation считает `preds=logits.argmax(dim=1)`, per-class `precision`, `recall`, `f1`, `iou`, `support_pixels`, `pred_pixels` для foreground классов `1..N`, а также `val_macro_f1`, `val_mean_iou`, `val_pixel_accuracy`. Для совместимости `val_pixel_f1` в multiclass равен `val_macro_f1`; best checkpoint и early stopping используют это же значение. Имена классов в метриках берутся из `TrainConfig.class_slugs`; если slug не передан, используется `class_{id}`.

...
Pretrained загрузка выполняется только при `spec.pretrained=true` с `ignore_mismatched_sizes=True`. Checkpoint `.pt` хранит `model_state_dict`, `model_spec`, `metadata`. Если `LoadCheckpointRequest.model_spec` задан, `load_checkpoint` сверяет его со спецификацией checkpoint и отклоняет несовпадение, включая число входных каналов. `load_checkpoint` работает с локальными путями; S3 и MLflow URI здесь не добавляются.

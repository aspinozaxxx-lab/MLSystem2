...
Training UI автоматически использует этот Geoalert/Triton-контракт для всех моделей ортофото: backend `geoalert_workflow_engine` кеширует GPU-export конкретного checkpoint, запускает штатный `urban.Compose` и выгружает модель из Triton после задания. Это относится к полной и поснимочной псевдоразметке, AOI и тестовому F1. Модели Канопуса пока сохраняют совместимый `pytorch_one_off`. Ниже остаётся ручной runbook для явного production-инференса, диагностики и отдельного экспорта.

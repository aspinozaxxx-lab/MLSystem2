...
`prepared_images/kanopus` или `prepared_images/orto` и возвращает временный UTF-8 TXT с именами файлов без
расширений. Файл не сохраняется в БД или постоянном хранилище; его имя повторяет имя загруженного GeoJSON с
расширением `.txt`, включая кириллицу. Имя выбранной сцены должно быть уникально во всём корне снимков
выбранного типа, иначе TXT был бы неоднозначным и операция завершается ошибкой.

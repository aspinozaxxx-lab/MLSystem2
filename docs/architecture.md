...
число окрашено тем же цветом, что и соответствующий контур. Черновик каждого снимка хранится в Postgres
...
снимки, создаёт один атомарный commit. Добавление папки пропускает уже присутствующие GeoJSON и создаёт один commit
на все новые файлы; в браузере снимков уже добавленные TIFF остаются в исходном порядке, показываются зелёной
рамкой и не имеют повторного выбора. Автор фиксирован сервисный,
...
сканирования каталога. Ошибочное задание повторяется только явным действием пользователя. Готовый GeoJSON
показывается отдельным нередактируемым слоем.

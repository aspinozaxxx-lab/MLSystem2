...
8. `train` выполняет PyTorch обучение segmentation-модели: AdamW, cosine scheduler, binary BCE/Dice-family loss или multiclass cross entropy, validation metrics, early stopping и best/final checkpoints. Checkpoint metadata содержит validation threshold и `sample_size`, необходимый для экспорта в Triton.

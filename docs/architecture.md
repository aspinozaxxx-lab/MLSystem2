...
Экспорт тестовой разметки является самостоятельной функцией `training_ui_api` и не входит в конвейер обучения,
очереди или автоматизацию. Он читает исходные TIFF только из `MLSYSTEM2_IMAGES_ROOT` с серверным значением
...
внутри `training_ui_api`, не пишет MLflow и не меняет конвейер обучения.

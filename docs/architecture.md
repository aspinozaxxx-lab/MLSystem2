...
При сохранённом расчёте каждый тайл также хранит пиксельный и объектовый F1 той же псевдоразметки. В карточке
тайла API и frontend показывают одно значение согласно `quality_metric` тестовой разметки; для старых записей
отсутствующий кэш заполняется один раз при открытии подробной страницы.

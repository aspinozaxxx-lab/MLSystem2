...
Редактор per-image датасета по умолчанию не показывает псевдоразметку. По явному переключателю он сначала
берёт последнюю успешную полную псевдоразметку текущей основной сети, чей список сцен содержит выбранный TIFF,
независимо от исходного датасета или папки результата, и обрезает её по valid-data footprint. Нормализация ссылки
...
оптимизации их состава и AOI-инференсом QGIS. Редактор тестовой разметки явно показывает имя сети и датасет её

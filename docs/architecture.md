...
снимки, создаёт один атомарный commit. Добавление папки пропускает уже присутствующие GeoJSON, создаёт разметку и
footprint одним commit; публикация удаления убирает оба файла. В браузере снимков уже добавленные TIFF остаются в исходном порядке, показываются зелёной
...
GeoJSON.
Удаление всего датасета доступно из редактора классов: после проверки отсутствия активных заданий его папка
удаляется отдельным Git-коммитом, основной датасет класса снимается, а строка `datasets` сохраняется с временем
удаления. Live-каталог переключается только штатной атомарной публикацией MLMarkup.

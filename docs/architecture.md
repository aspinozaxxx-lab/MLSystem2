...
корня типа снимков по пересечению, обрабатывает пересекающие AOI окна, векторизует отдельные связные компоненты
и сохраняет обрезанный GeoJSON с площадью и средней пиксельной уверенностью объекта в `stored_files`.

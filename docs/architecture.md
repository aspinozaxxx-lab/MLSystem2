...
Групповое скачивание принимает явный список сохранённых разметок. Основные разметки предварительно выбраны
только в frontend; backend не подменяет переданный список. Каждая разметка готовится в отдельном временном
каталоге задачей пула до восьми потоков без общей SQLAlchemy-сессии и без общего `ZipFile`, после чего один
поток детерминированно собирает общий ZIP без сжатия (`ZIP_STORED`) с отдельной папкой каждой разметки.

...
число окрашено тем же цветом, что и соответствующий контур. Полноширинный
...
MLflow-run на GPU. Обычный running-инференс не прерывается.
...
`pytorch_one_off` runner только для этого TIFF; готовый GeoJSON показывается отдельным нередактируемым слоем.

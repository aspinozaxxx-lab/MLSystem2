...
`prepared_images/kanopus` или `prepared_images/orto` и возвращает временный UTF-8 TXT с полными относительными
путями внутри выбранного корня без расширений. Файл не сохраняется в БД или постоянном хранилище; его имя
повторяет имя загруженного GeoJSON с расширением `.txt`, включая кириллицу. Совпадающие имена TIFF в разных
папках различаются путём; совпадение относительного пути без расширения считается неоднозначностью.

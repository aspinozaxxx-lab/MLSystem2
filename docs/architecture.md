...
через общую inference-очередь и тот же `pytorch_one_off` runner. Клиент передаёт только класс, Polygon или
MultiPolygon и CRS. Сервер фиксирует последнюю пригодную модель основного датасета, выбирает TIFF только внутри
корня типа снимков по пересечению, обрабатывает пересекающие AOI окна и расширяет соседний контекст до окончания
связных объектов. Задание публикуется только после успешной обработки всех выбранных снимков. Полностью внешние
объекты удаляются, а пересекающие AOI сохраняются целиком с площадью и средней пиксельной уверенностью в `stored_files`.
...
в локальном GeoPackage staging; единый Qt undo stack не объединяется с независимыми undo stack целевых слоёв.

...
берёт последнюю успешную полную псевдоразметку точного датасета текущей основной сетью и обрезает её по valid-data
footprint открытого TIFF. Если результат не покрывает снимок, создаётся идемпотентное срочное задание того же
`pytorch_one_off` runner только для этого TIFF; уникальный ключ включает сеть, TIFF и effective inference-конфигурацию.

...
авторизованный GeoTIFF через HTTP Range без экранной интерполяции пикселей и позволяет Draw/Modify/Snap, смену роли, удаление и undo. Полноширинный
...
полигональные объекты с географическими контурами TIFF только внутри соответствующего корня
...
различаются путём; совпадение относительного пути без расширения считается неоднозначностью. Старый HTTP-режим

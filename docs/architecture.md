...
`training_ui_api`: для итоговых `N` тайлов и минимальных `M` объектов строится пул до `3N` с целью `3M`, после
чего по последней точной псевдоразметке включается оптимальный состав ровно из `N` тайлов. Состояние запуска и
...
включённый TIFF-тайл. TP/FP/FN суммируются без объединения совпадающих географических областей. Метрика хранится
в Postgres, не пишется в MLflow и считается актуальной только для тех же `sample_id`, ревизии состава и версии
эффективного inference-шаблона. Новая успешная сеть ставится на оценку автоматически; смена основной выборки или
её состава требует пересчёта.

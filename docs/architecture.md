...
варианта MLMarkup и на один час сохраняет TIFF, GeoJSON, бинарную PNG-маску, PNG-превью и плоский ZIP во временном
подкаталоге `scratch_root`. Записи в Postgres и MLflow и обращения к S3 при этом не выполняются.

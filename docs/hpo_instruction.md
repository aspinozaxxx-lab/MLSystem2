...
- анализировать предыдущие результаты hpo сессий в папке /opt/hpo/report/ и данные в mlflow и строить стратегию на основе уже проделанной работы
- нужно учитывать что датасеты меняются со временем (видно по коммитам в репо с разметками). Это не полностью отменяет HPO, но дает потенциал к обновлению при необходимости ключевых опорных знаний.
...
- report workspace: `/opt/hpo/report/{датасет (например defrostation)}_{ключевые особености (например архитекутура сети + ключевой парметр)}_{DDMM (начала hpo сессии)}`;
...
- `errors.md`;
- `changes.md`.
- `trials_journal.md` - список всех опртобованных комбинаций параметров с результатам по пиксельной f1 score, количеством эпох и времением обучения. 
...
- `positive_factor`, `hard_negative_factor`, `background_factor` использовать как согласованную тройку HPO-параметров с суммой `1.0`;
